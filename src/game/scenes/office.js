...
import { CLOTHES_COLORS, generateUUID, HAIR_COLORS, SKIN_COLORS, snap, TILE_DIMENSION } from '../utils/misc';
...
const getObjects = (map, type) =>
  map.filterObjects('Objects', (o) => {
...
export default class OfficeScene extends Phaser.Scene {
  ...
  create() {
    ...
    const _underLayer = this.map.createStaticLayer('Below', this.tileset, 0, 0);
  ...
  addEmployee(storedMeta = null) {
    ...
    const meta = storedMeta || {
      ...
      age: randRange(18, 50),
      hobbies: [randRange(1, 3), 1].map(() => randValue(HOBBIES)),
  ...
  findReliefPoint(reliefId) {
    ...
    const cleanAndEmptyPoints = points.filter((p) => !p.busy && !p.meta.broken);
